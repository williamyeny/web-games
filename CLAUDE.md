...
- `plus/` holds this edition's features (the stage pictures in scene.js, trophies, multiverse, menu, sounds). Shared state is `window.UP`, saved under the `up-plus` key. `ui.js` arranges the screen for phones.
